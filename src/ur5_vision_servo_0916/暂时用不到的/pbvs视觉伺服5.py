...
不使用input单步调试了，启用连续跟踪
这个使用time.sleep(1.5)让机械臂最后对准标定板中心的时候不至于来回震荡，但是还是不太行，还要再改
"""
import numpy as np
import cv2
import json
...
def unlocked_pose_visual_servoing_final():
    ...
    Z_SAFE_LIMIT = 0.05
    LOOP_INTERVAL = 0.1  # 循环间隔0.1秒，即追踪频率为 10Hz
    MOVE_SPEED = 0.3  # 机器人移动速度 (0.1 ~ 1.0)
    MOVE_ACCEL = 0.3  # 机器人移动加速度 (0.1 ~ 1.0)
            ...
            if rvec is None:
                ...
                if error > THRESHOLD:
                    if pos[2] < Z_SAFE_LIMIT:
                        ...
                        try:
                            robot.move_j_p_1(target_pose_xyzrpy.tolist(), k_acc=MOVE_ACCEL, k_vel=MOVE_SPEED)
                            time.sleep(1.5)
