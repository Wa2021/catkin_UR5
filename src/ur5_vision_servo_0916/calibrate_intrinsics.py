#手眼标定1.4中每次运行都会重新计算一次内参，而
#相机内参是相机的固有属性，一旦标定好，只要你不更换镜头、不改变焦距、不剧烈磕碰相机，它就是固定不变的。
# 完全没有必要在每次手眼标定时都重新计算一次，所以可以单独进行内参标定，配合手眼标定1.5使用（还没用上
...
class CameraIntrinsicsCalibrator:
    def __init__(self, pattern_size=(8, 5), square_size=0.027):
        ...
        self.output_dir = "calibration_images"  # 用于存储拍摄的图像
        self.output_file = "camera_intrinsics.json"  # 最终结果保存的文件名
    ...
    # 在你的 CameraIntrinsicsCalibrator 类中，替换掉旧的 capture_images_from_camera 方法

    def capture_images_from_camera(self, camera_index=0, num_images=30):
        """
        从连接到PC的摄像头实时捕获图像用于标定。
        【V2版：带有可视化辅助界面，实时显示角点分布】
        ...
        while count < num_images:
            ...
            cv2.imshow('Coverage Map', coverage_map)  # 显示角点分布图
            ...
            if key == ord('q'):
                ...
                # 按下 'c' 键时，我们用更高精度的设置再找一次角点
                found_capture, corners_capture = cv2.findChessboardCorners(gray, self.pattern_size, None)
                ...
                if found_capture:
                    ...
                    for corner in corners_capture:
                        # corner[0][0]是x, corner[0][1]是y
                        # 我们加上50的偏移，因为我们的分布图比实际画面大
                        pt_x = int(corner[0][0]) + 50
                        pt_y = int(corner[0][1]) + 50
                        cv2.circle(coverage_map, (pt_x, pt_y), 3, (0, 255, 255), -1)  # 黄色的点

                    # 更新捕获到的角点列表，用于之后的高精度计算
    ...
    def calibrate_from_images(self):
        ...
        image_files = [os.path.join(self.output_dir, f) for f in os.listdir(self.output_dir) if
        ...
        # 保存结果
        self.save_intrinsics(camera_matrix, dist_coeffs, reprojection_error)

    def save_intrinsics(self, camera_matrix, dist_coeffs, error):
        """
        将标定结果保存到JSON文件。
        """
        intrinsics_data = {
